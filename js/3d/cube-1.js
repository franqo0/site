...
const cube1Container = document.getElementById("cube-1");
...
// Luz ambiental
const ambient_light = new THREE.AmbientLight('#FFFFFF', 0.5)
scene.add(ambient_light)
// Punto de luz
const light = new THREE.PointLight('#FFFFFF', 0, 6000)
...
loader.load('models/FLAT-CUBE.glb', (gltf) => {
...
// Escalar el modelo proporcionalmente al ancho del contenedor
    const factorScale = 28;
        model.scale.set(factorScale, factorScale, factorScale);
        model.position.set((modelX - centerX) / 8.02 + 5, (modelY + centerY) / 45.3, 0);
...
// Rotar modelo
    const factorRotation = 0.005
        model.rotation.x += factorRotation
        model.rotation.y += factorRotation
        model.rotation.z += factorRotation
}
    animate();
}, undefined, (error) => {
    console.error(error)
